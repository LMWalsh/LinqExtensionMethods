    ...
    class Program {
        static void Main(string[] args) {
           ...
           // var total = nbrs2.Sum();
            ...
            var SumDivBy3 = nbrs2.Where(n => n % 3 == 0).Sum();  //method syntax - gives same answer
            Console.WriteLine($" Sum divided by 2: {SumDivBy3}"); //as declarative below

            var nbrsSumDivBy3Ext = from n in nbrs2   //declarative syntax
                                   where n % 3 == 0  //does not execute here - just defining
                                   select n;          //exec when access var nbrsSumDivBy3Ext
            foreach(var n in nbrsSumDivBy3Ext) {
                Console.Write($" {n} ");
            }


            var nbrsAvg = nbrs2.Average();
            Console.WriteLine($" Average : {nbrsAvg}");
        }
    }
}
